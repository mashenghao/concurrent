 ...
 * @date: 2019/8/15
 */
public class ThreadPool3 extends Thread {

    /*
        扩容与回收
        1.更改线程池也为线程，当初始化线程完毕后，启动扩容方法
        2.扩容：
            1.当TASK_QUEUE大于当前线程数量，扩容到指定大小
            2.当TASK_QUEUE为空时，减少工作线程数量
    ...
    public static final ThreadPool3.DiscardPolicy default_discardPolicy = (task) -> {
        throw new ThreadPool3.DiscardException("discard the submit and throw exception... ");
    };
    ...
    private final ThreadPool3.DiscardPolicy discardPolicy;
     ...
     */
    public ThreadPool3(int min, int active, int max, int taskQueueSize, DiscardPolicy discardPolicy) {
        this.size = size;
    ...
    public void shutdown() throws InterruptedException {
        ...
        while (size > 0) {
            for (ThreadPool3.WorkThread w : WORK_THREAD_QUEUE) {
                if (w.statu == ThreadPool3.Status.WAITING) {
                    w.interrupt();
                    w.close();
                    size--;
                } else {
    ...
    //创建工作线程
    private ThreadPool3.WorkThread createWorkThread() {
        ThreadPool3.WorkThread workThread = new ThreadPool3.WorkThread(WORK_THREAD_PREFIX + (seq++));
    ...
    private class WorkThread extends Thread {
        ...
        public ThreadPool3.Status statu = ThreadPool3.Status.FREE;//初始创建是空闲的
        ...
        public void run() {
            //当线程未被关闭是，就一直运行
            OUTLINE:
            while (this.statu != ThreadPool3.Status.DEAD) {
                synchronized (TASK_QUEUE) {
                    while (TASK_QUEUE.isEmpty()) {//任务队列中为空，线程等待
                        try {
                            //System.err.println(ThreadUtil.getName() + " be waiting, status: " + statu);
                            this.statu = ThreadPool3.Status.WAITING;//为以后中断线程使用，标记线程可中断，没有任务执行
                            TASK_QUEUE.wait();
                        } catch (InterruptedException e) {
                ...
                if (task != null) {
                    this.statu = ThreadPool3.Status.RUNNING;
                    task.run();//运行任务
                    //运行结束后，任务值为空，线程状态置为空闲
                    this.statu = ThreadPool3.Status.FREE;
                    task = null;
                }
        ...
        public void close() {
            ...
            this.statu = ThreadPool3.Status.DEAD;
        }
    }
    ...
    public interface DiscardPolicy {
        void discard(Runnable task) throws ThreadPool3.DiscardException;
    }
...
class TestThreadPool3 {

    public static void main(String[] args) throws InterruptedException {
        ThreadPool3 threadPool3 = new ThreadPool3(3, 10, 20, 200, ThreadPool3.default_discardPolicy);
